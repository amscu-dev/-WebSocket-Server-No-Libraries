...
export default class WebSocketParser extends EventEmitter {
   ...
   * This keeps WebSocketParser instance in memory while socket is active.
   *
   * Memory Flow:
   * 1. upgradeHttpConnection() creates WebSocketParser instance
   ...
   * 5. WebSocketParser stays in memory (closure holds `this`) ✅
   * 6. socket.destroy() removes listeners, breaks closure
   * 7. WebSocketParser can be garbage collected ✅
   *
   * @private
   */
...
│              ├─ this (WebSocketParser instance) │
│              ├─ socket (TCP Socket)             │
│              └─ chunk (function parameter)      │
│                                                  │
└──────────────────────────────────────────────────┘

WebSocketParser stays in memory BECAUSE the closure holds a reference to `this`
*/
  }
